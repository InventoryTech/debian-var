...
# $1 - rootfs base dir
function make_debian_minimal_rootfs() {
	local ROOTFS_BASE=$1

	pr_info "Make debian(${DEB_RELEASE}) minimal rootfs start..."
	...
	debootstrap --verbose  --variant=minbase --include=ifupdown --no-check-gpg --foreign --arch armhf ${DEB_RELEASE} \
...
# network manager & QMI support
protected_install network-manager
protected_install libqmi-utils
...
# Must be called after make_debian_minimal_rootfs in main script
# function generate ubi rootfs in input dir
# $1 - rootfs ubifs base dir
function prepare_minimal_ubifs_rootfs() {
	local UBIFS_ROOTFS_BASE=$1
	pr_info "Make debian(${DEB_RELEASE}) minimal rootfs for UBIFS start..."
